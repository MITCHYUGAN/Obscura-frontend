...
import { parseAmount, isValidAmount, isValidAddress, formatAmount } from "@/lib/contracts";
...
export function TransferForm() {
  ...
  return (
    <div className="rounded-2xl border border-border/50 bg-card p-6 space-y-4">
      <div>
        <h3 className="font-semibold text-base">Private Transfer</h3>
        <p className="text-sm text-muted-foreground mt-0.5">
          Send strkBTC to another address inside the pool. No on-chain trace.
        </p>
      </div>

      {/* Privacy badge */}
      <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-950/30 border border-emerald-800/30">
        <Shield className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
        <p className="text-xs text-emerald-400/80">
          This transfer emits <strong className="text-emerald-300">zero events</strong>.
          Observers cannot see it happened at all.
        </p>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <input
          ...
          className="w-full h-11 px-3 rounded-lg border border-border bg-input font-mono text-sm text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 disabled:opacity-40"
        />
      ...
      <button
        ...
        className="w-full h-11 flex items-center justify-center gap-2 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-black font-semibold text-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
      >
        {isBusy ? (
          <>
