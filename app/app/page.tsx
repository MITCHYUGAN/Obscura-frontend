        ...
        <div className="text-vault-400">
          ...
          {tx.hash && (
            <a href={explorerTx(tx.hash)} target="_blank" rel="noopener noreferrer"
              className="text-xs font-mono text-vault-500 hover:text-vault-300 transition-colors mt-1 flex items-center gap-1">
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-64 border-r border-border/50 bg-card flex flex-col z-40">
      ...
      <div className="h-16 flex items-center gap-2.5 px-5 border-b border-border/40">
        ...
        <div>
          <span className="font-display font-bold text-lg">Obscura</span>
          <span className="ml-1.5 text-xs font-mono text-muted-foreground">sepolia</span>
        </div>
      </div>
        ...
        {NAV.map(({ id, icon, label }) => (
          <button
            ...
            className={cn(
              ...
              tab === id
                ? "bg-vault-500/10 text-vault-400 border border-vault-700/30"
        ...
        <div className="pt-5 mt-auto">
          ...
          <button
            ...
            className={cn(
              "w-full flex items-center gap-3 px-3 py-3 rounded-lg text-base font-medium transition-all",
              tab === "faucet"
                ? "bg-orange-500/10 text-orange-400 border border-orange-700/30"
                : "text-muted-foreground hover:text-foreground hover:bg-secondary/60"
            )}
          >
      ...
      <div className="p-3 border-t border-border/40">
        ...
        <Link
          href="/"
          className="flex items-center gap-1.5 mt-2 px-2 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors rounded-lg hover:bg-secondary/40"
        >
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="rounded-xl border border-border/50 bg-card p-4">
            ...
            <p className={cn("font-mono font-bold text-2xl tabular text-foreground", isLoadingBalances && "opacity-40")}>
    ...
    <div className="rounded-2xl border border-border/50 bg-card p-7 space-y-6">
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          value={recipient}
          onChange={e => { setRecipient(e.target.value); if (tx.status !== "idle") setTx({ status: "idle" }); }}
          disabled={isBusy}
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          <input
            ...
            value={amount}
            onChange={e => { setAmount(e.target.value); if (tx.status !== "idle") setTx({ status: "idle" }); }}
            disabled={isBusy}
            className="w-full h-14 px-4 pr-24 rounded-xl border border-border bg-input font-mono text-xl text-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:ring-2 focus:ring-ring transition-colors disabled:opacity-40"
          />
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-muted-foreground font-mono">strkBTC</span>
        ...
        <div className="grid grid-cols-4 gap-2">
          {["0.1", "0.5", "1", "10"].map(v => (
            <button
              key={v}
              onClick={() => { setAmount(v); if (tx.status !== "idle") setTx({ status: "idle" }); }}
              disabled={isBusy}
              className="py-2.5 text-sm rounded-lg bg-secondary hover:bg-muted border border-border text-muted-foreground hover:text-foreground transition-colors disabled:opacity-40 font-medium"
            >
              {v}
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <main className="flex-1 ml-64 min-h-screen">
        ...
        <div className="mx-8 mt-5 px-5 py-3 rounded-xl border border-amber-800/30 bg-amber-950/15 text-base text-amber-500/70 flex items-center gap-2">
          ...
          <strong className="text-amber-500/90">Experimental testnet software.</strong>
          {" "}Do not use with real funds. Contracts are unaudited.
        </div>
